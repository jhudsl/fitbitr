...
noDataQuery <- readRDS(here("tests/testthat/queryReturns/noDataIntradayQuery.rds"))
...
test_that("We can handle when a query doesn't have any data in it.", {
  expect_equal(extractIntraday(noDataQuery), dplyr::data_frame(type = character(), time = numeric(), value = numeric()))
})
